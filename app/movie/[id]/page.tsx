...
const MovieDetailPage: React.FC<{ params: Promise<{ id: string }> }> = ({ params }) => {
  ...
  return (
    <main className="min-h-screen bg-[#0f172a] text-white">
      ...
      <div className="relative w-full aspect-video md:h-[70vh] bg-black overflow-hidden">
        {movie.trailerId ? (
          <iframe
            width="100%"
            height="100%"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12 lg:p-20 xl:p-28 space-y-4 max-w-6xl">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
          >
            <Link href="/" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors mb-6 text-sm font-medium">
              <ChevronLeft size={16} /> Kembali
            </Link>
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-white/90 mb-4 leading-tight">
              {movie.title}
            </h1>
            ...
            <div className="flex flex-wrap gap-4">
              <button 
                ...
                className="flex items-center gap-2 px-8 py-3 bg-white/90 text-black/90 rounded-xl font-bold hover:bg-white transition-all active:scale-95"
              >
                <Play size={20} fill="black" /> Mulai Sekarang
              </button>
              <button className="flex items-center gap-2 px-8 py-3 glass text-white rounded-xl font-bold hover:bg-white/20 transition-all active:scale-95">
      ...
      {/* Content Section */}
      <div className="max-w-[1920px] mx-auto px-6 md:px-12 lg:px-20 xl:px-28 py-16 grid md:grid-cols-3 gap-12">
        <div className="md:col-span-2 space-y-8">
          <section>
            <h2 className="text-2xl font-bold mb-4 border-l-4 border-purple-600 pl-4 text-white/80">Deskripsi</h2>
          ...
          {movie.imdbLink || movie.tomatoLink ? (
            <section>
              <h2 className="text-2xl font-bold mb-4 border-l-4 border-purple-600 pl-4 text-white/80">Rating & Link Eksternal</h2>
              <div className="flex flex-wrap gap-4">
                {movie.imdbLink && (
                  <a 
                    href={movie.imdbLink} 
                    ...
                    className="flex items-center gap-2 px-6 py-3 glass rounded-xl text-white hover:bg-white/20 transition-all text-sm font-medium"
                  >
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-bold border-l-4 border-purple-600 pl-4 text-white/80">Poster</h2>
          <div className="relative aspect-[2/3] rounded-3xl overflow-hidden apple-shadow group">
            <Image 
              src={imgSrc} 
              alt={movie.title} 
              fill 
      ...
      <footer className="mt-20 px-6 md:px-12 lg:px-20 xl:px-28 py-16 border-t border-white/10 flex flex-col md:flex-row justify-between gap-12 md:gap-8">
        <div className="space-y-4">
          ...
          <p className="text-gray-400 text-sm">© 2023 Chill. All Rights Reserved. | Template developed by Rohman Developer.</p>
        </div>
